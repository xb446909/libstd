		...
		int stream_size)
{
	...
	if (state == Z_NULL)
		return Z_MEM_ERROR;Tracev((stderr, "inflate: allocated\n"));
...
int ZEXPORT inflate(z_streamp strm, int flush)
{
	...
	for (;;)
		...
		case CHECK:
			if (state->wrap)
			{
				...
				state->total += out;
				if ((state->wrap & 4) && out)
					strm->adler = state->check = UPDATE(state->check, put - out,
							out);
				out = left;
				if ((state->wrap & 4) && (
#ifdef GUNZIP
	...
	state->total += out;
	if ((state->wrap & 4) && out)
		strm->adler = state->check = UPDATE(state->check, strm->next_out - out,
				out);
