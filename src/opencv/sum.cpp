...
struct Sum_SIMD<schar, int>
{
			...
			int cn) const
	{
		...
		for (; x <= len - 16; x += 16)
		{
			...
			__m128i v_half = _mm_srai_epi16(_mm_unpacklo_epi8(v_zero, v_src),
					8);

			v_sum = _mm_add_epi32(v_sum,
					_mm_srai_epi32(_mm_unpacklo_epi16(v_zero, v_half), 16));
			v_sum = _mm_add_epi32(v_sum,
					_mm_srai_epi32(_mm_unpackhi_epi16(v_zero, v_half), 16));
		...
		for (; x <= len - 8; x += 8)
		{
			__m128i v_src = _mm_srai_epi16(
					_mm_unpacklo_epi8(v_zero,
							_mm_loadl_epi64((__m128i const *) (src0 + x))), 8);

			v_sum = _mm_add_epi32(v_sum,
					_mm_srai_epi32(_mm_unpacklo_epi16(v_zero, v_src), 16));
			v_sum = _mm_add_epi32(v_sum,
					_mm_srai_epi32(_mm_unpackhi_epi16(v_zero, v_src), 16));
		}
...
struct Sum_SIMD<int, double>
{
			...
			int cn) const
	{
		...
		for (; x <= len - 4; x += 4)
		{
			...
			v_sum1 = _mm_add_pd(v_sum1,
					_mm_cvtepi32_pd(_mm_srli_si128(v_src, 8)));
		}
...
{
	int operator ()(const float * src0, const uchar * mask, double * dst,
			int len, int cn) const
	{
		...
		for (; x <= len - 4; x += 4)
		{
			...
			v_src = _mm_castsi128_ps(
					_mm_srli_si128(_mm_castps_si128(v_src), 8));
...
SumFunc getSumFunc(int depth)
{
	...
	{ (SumFunc) GET_OPTIMIZED(sum8u), (SumFunc) sum8s, (SumFunc) sum16u,
			(SumFunc) sum16s, (SumFunc) sum32s, (SumFunc) GET_OPTIMIZED(sum32f),
			(SumFunc) sum64f, 0 };
		...
		InputArray _src2, bool calc2, const Scalar & res2 )
{
	...
	static const char * const opMap[3] =
	{	"OP_SUM", "OP_SUM_ABS", "OP_SUM_SQR"};
	char cvt[2][40];
	...
	if (k.run(1, &globalsize, &wgs, false))
	{
		...
		part_sum funcs[3] =
		{	ocl_part_sum<int>, ocl_part_sum<float>, ocl_part_sum<double>},
