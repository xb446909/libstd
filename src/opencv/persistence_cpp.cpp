...
static void getElemSize(const String& fmt, size_t& elemSize, size_t& cn)
{
	...
	char c = dt[0];
	elemSize = cn
			* (c == 'u' || c == 'c' ? sizeof(uchar) :
				c == 'w' || c == 's' ? sizeof(ushort) : c == 'i' ? sizeof(int) :
				c == 'f' ? sizeof(float) : c == 'd' ? sizeof(double) :
...
{
	return isOpened() ?
			FileNode(fs, cvGetRootFileNode(fs, streamidx)) : FileNode();
}
...
FileStorage& operator <<(FileStorage& fs, const String& str)
{
	...
	if (*_str == '}' || *_str == ']')
	{
		...
		fs.structs.pop_back();
		fs.state =
				fs.structs.empty() || fs.structs.back() == '{' ?
	...
	else if ((fs.state & 3) == VALUE_EXPECTED)
	{
		if (*_str == '{' || *_str == '[')
		{
			...
			cvStartWriteStruct(*fs,
					fs.elname.size() > 0 ? fs.elname.c_str() : 0, flags,
					*_str ? _str : 0);
			fs.elname = String();
		}
		else
...
{
	return isSeq() ?
			FileNode(fs, (CvFileNode*) cvGetSeqElem(node->data.seq, i)) :
			i == 0 ? *this : FileNode();
}
...
{
	const char* str;
	return !node || (str = cvGetFileNodeName(node)) == 0 ?
			String() : String(str);
}
...
static const FileNodeIterator::SeqReader emptyReader =
{ 0, 0, 0, 0, 0, 0, 0, 0 };
		...
		const CvFileNode* _node, size_t _ofs)
{
	...
	if (_fs && _node && CV_NODE_TYPE(_node->tag) != CV_NODE_NONE)
	{
		...
		fs = _fs;
		container = _node;
		if (!(_node->tag & FileNode::USER)
				&& (node_type == FileNode::SEQ || node_type == FileNode::MAP))
		{
...
FileNodeIterator& FileNodeIterator::operator ++()
{
	if (remaining > 0)
	{
		if (reader.seq)
		{
			if (((reader).ptr += (((CvSeq*) reader.seq)->elem_size))
					>= (reader).block_max)
			{
...
FileNodeIterator& FileNodeIterator::readRaw(const String& fmt, uchar* vec,
		size_t maxCount)
{
	if (fs && container && remaining > 0)
	{
		...
		if (reader.seq)
		{
			cvReadRawDataSlice(fs, (CvSeqReader*) &reader, (int) count, vec,
					fmt.c_str());
			remaining -= count * cn;
		}
		else
...
internal::WriteStructContext::WriteStructContext(FileStorage& _fs,
		const String& name, int flags, const String& typeName) :
		fs(&_fs)
{
...
void read(const FileNode& node, SparseMat& mat, const SparseMat& default_mat)
{
	...
	Ptr<CvSparseMat> m(
			(CvSparseMat*) cvRead((CvFileStorage*) node.fs,
					(CvFileNode*) *node));
...
void read(const FileNode& node, int& value, int default_value)
{
	value = !node.node ? default_value :
			CV_NODE_IS_INT(node.node->tag) ?
					node.node->data.i : std::numeric_limits<int>::max();
}
...
void read(const FileNode& node, float& value, float default_value)
{
	value = !node.node ? default_value :
			...
			CV_NODE_IS_REAL(node.node->tag) ?
					saturate_cast<float>(node.node->data.f) :
...
void read(const FileNode& node, double& value, double default_value)
{
	value = !node.node ? default_value :
			...
			CV_NODE_IS_REAL(node.node->tag) ?
					node.node->data.f : std::numeric_limits<double>::max();
}

void read(const FileNode& node, String& value, const String& default_value)
{
	value = !node.node ? default_value :
			CV_NODE_IS_STRING(node.node->tag) ?
					String(node.node->data.str.ptr) : String();
}

void read(const FileNode& node, std::string& value,
		const std::string& default_value)
{
	value = !node.node ? default_value :
			CV_NODE_IS_STRING(node.node->tag) ?
					std::string(node.node->data.str.ptr) : default_value;
}
