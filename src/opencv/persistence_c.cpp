...
static inline bool cv_strcasecmp(const char * s1, const char * s2)
{
	...
	for (size_t i = 0U; i < len1; i++)
		if (tolower(static_cast<int>(s1[i]))
				!= tolower(static_cast<int>(s2[i])))
			return false;
...
static std::vector<std::string> analyze_file_name(std::string const & file_name)
{
	...
	{
		beg++;
		for (size_t param_beg = beg, param_end = beg; param_end < end;
				param_beg = param_end + 1U)
		{
			param_end = file_name.find_first_of(parameter_separator, param_beg);
			if ((param_end == std::string::npos || param_end != param_beg)
					&& param_beg + 1U < end)
			{
				result.push_back(
						file_name.substr(param_beg, param_end - param_beg));
			}
		}
	}
...
static bool is_param_exist(const std::vector<std::string> & params,
...
cvOpenFileStorage(const char* query, CvMemStorage* dststorage, int flags,
		const char* encoding)
{
	CvFileStorage* fs = 0;
	...
	bool write_base64 = (write_mode || append)
			&& (flags & CV_STORAGE_BASE64) != 0;
	bool isGZ = false;
	size_t fnamelen = 0;
	...
	if (!filename || filename[0] == '\0')
	{
		if (!write_mode)
			CV_Error(CV_StsNullPtr,
					mem ? "NULL or empty filename" : "NULL or empty buffer");
		mem = true;
	}
	else
	...
	if (mem && append)
		CV_Error(CV_StsBadFlag,
				"CV_STORAGE_APPEND and CV_STORAGE_MEMORY are not currently compatible");
	...
	if (!mem)
	{
		...
		if (dot_pos && dot_pos[1] == 'g' && dot_pos[2] == 'z'
				&& (dot_pos[3] == '\0'
						|| (cv_isdigit(dot_pos[3]) && dot_pos[4] == '\0')))
		{
			if (append)
			{
...
#if USE_ZLIB
			char mode[] =
			{ fs->write_mode ? 'w' : 'r', 'b', compression ? compression : '3',
					'\0' };
	...
	if (fs->write_mode)
	{
		...
		if (fmt == CV_STORAGE_FORMAT_AUTO && filename)
		{
			...
			fs->fmt =
					(cv_strcasecmp(dot_pos, ".xml")
							|| cv_strcasecmp(dot_pos, ".xml.gz")) ?
					CV_STORAGE_FORMAT_XML :
					(cv_strcasecmp(dot_pos, ".json")
							|| cv_strcasecmp(dot_pos, ".json.gz")) ?
		...
		int buf_size = CV_FS_MAX_LEN
				* (fs->fmt == CV_STORAGE_FORMAT_XML ? 6 : 4) + 1024;
	...
	else
	{
		...
		fs->roots = cvCreateSeq(0, sizeof(CvSeq), sizeof(CvFileNode),
				fs->memstorage);
	...
	{
		if (cvGetErrStatus() < 0
				|| (!fs->file && !fs->gzfile && !fs->outbuf && !fs->strbuf))
		{
			cvReleaseFileStorage(&fs);
		}
		...
		const char** _real_name)
{
	...
	cv::FileStorage fs(
			cvOpenFileStorage(filename, memstorage, CV_STORAGE_READ));
...
cvGetHashedKey(CvFileStorage* fs, const char* str, int len, int create_missing)
{
	...
	for (node = (CvStringHashNode*) (map->table[i]); node != 0;
			node = node->next)
	{
		if (node->hashval == hashval && node->str.len == len
...
cvGetFileNode(CvFileStorage* fs, CvFileNode* _map_node,
		const CvStringHashNode* key, int create_missing)
{
	CvFileNode* value = 0;
	int k = 0, attempts = 1;
	...
	for (k = 0; k < attempts; k++)
	{
		...
		{
			if ((!CV_NODE_IS_SEQ(map_node->tag)
					|| map_node->data.seq->total != 0) &&
		...
		for (another = (CvFileMapNode*) (map->table[i]); another != 0; another =
				another->next)
			if (another->key == key)
			{
				if (!create_missing)
...
CV_IMPL CvFileNode*
cvGetFileNodeByName(const CvFileStorage* fs, const CvFileNode* _map_node,
		const char* str)
{
	CvFileNode* value = 0;
	int i, len, tab_size;
	...
	for (k = 0; k < attempts; k++)
	{
				...
				another->next)
		{
			...
			if (key->hashval == hashval && key->str.len == len
...
CV_IMPL void cvStartWriteStruct(CvFileStorage* fs, const char* key,
		int struct_flags, const char* type_name, CvAttrList /*attributes*/)
{
	...
	if (fs->state_of_writing_base64 == base64::fs::Uncertain
			&& CV_NODE_IS_SEQ(struct_flags) && fs->is_default_using_base64
			&& type_name == 0)
	{
	...
	else if (type_name && memcmp(type_name, "binary", 6) == 0)
	{
		...
		if (!CV_NODE_IS_SEQ(struct_flags))
			...
			CV_Error(CV_StsError,
					"function \'cvStartWriteStruct\' calls cannot be nested if using Base64.");
...
CV_IMPL void cvWriteString(CvFileStorage* fs, const char* key,
...
CV_IMPL void cvWriteComment(CvFileStorage* fs, const char* comment,
		int eol_comment)
{
	CV_CHECK_OUTPUT_FILE_STORAGE(fs);
...
CV_IMPL void cvWriteRawData(CvFileStorage* fs, const void* _data, int len,
		const char* dt)
{
...
CV_IMPL void cvStartReadRawData(const CvFileStorage* fs, const CvFileNode* src,
		CvSeqReader* reader)
{
	int node_type;
	CV_CHECK_FILE_STORAGE(fs);
...
CV_IMPL void cvReadRawDataSlice(const CvFileStorage* fs, CvSeqReader* reader,
		int len, void* _data, const char* dt)
{
	...
	end_loop: if (i != count - 1 || k != fmt_pair_count - 1)
		CV_Error(CV_StsBadSize,
...
CV_IMPL void cvReadRawData(const CvFileStorage* fs, const CvFileNode* src,
		void* data, const char* dt)
{
	...
	cvReadRawDataSlice(fs, &reader,
	CV_NODE_IS_SEQ(src->tag) ? src->data.seq->total : 1, data, dt);
}

CV_IMPL void cvWriteFileNode(CvFileStorage* fs, const char* new_node_name,
...
CV_IMPL void cvWrite(CvFileStorage* fs, const char* name, const void* ptr,
...
CV_IMPL void cvSave(const char* filename, const void* struct_ptr,
		const char* _name, const char* comment, CvAttrList attributes)
{
	...
	cv::String name =
			_name ? cv::String(_name) : cv::FileStorage::getDefaultObjectName(
							filename);
