...
void MatAllocator::download(UMatData* u, void* dstptr, int dims,
...
void MatAllocator::upload(UMatData* u, const void* srcptr, int dims,
...
void MatAllocator::copy(UMatData* usrc, UMatData* udst, int dims,
		const size_t sz[], const size_t srcofs[], const size_t srcstep[],
...
BufferPoolController* MatAllocator::getBufferPoolController(
		const char* id) const
{
	(void) id;
		...
		bool autoSteps)
{
	...
	size_t esz = CV_ELEM_SIZE(m.flags), esz1 = CV_ELEM_SIZE1(m.flags), total =
			esz;
...
void Mat::deallocate()
{
	if (u)
	{
		UMatData* u_ = u;
		u = NULL;
		(u_->currAllocator ? u_->currAllocator :
			allocator ? allocator : getDefaultAllocator())->unmap(u_);
	}
}
...
Mat::Mat(const Mat& m, const Rect& roi) :
		flags(m.flags), dims(2), rows(roi.height), cols(roi.width), data(
				m.data + roi.y * m.step[0]), datastart(m.datastart), dataend(
				m.dataend), datalimit(m.datalimit), allocator(m.allocator), u(
				m.u), size(&rows)
{
	CV_Assert(m.dims <= 2);
...
void Mat::push_back(const Mat& elems)
{
	...
	if (type() != elems.type())
		CV_Error(CV_StsUnmatchedFormats,
				"Pushed vector type is not the same as matrix type");
	...
	if (isContinuous() && elems.isContinuous())
		memcpy(data + r * step.p[0], elems.data,
				elems.total() * elems.elemSize());
	else
	{
...
int Mat::checkVector(int _elemChannels, int _depth,
