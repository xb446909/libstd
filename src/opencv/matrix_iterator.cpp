...
NAryMatIterator::NAryMatIterator() :
		arrays(0), planes(0), ptrs(0), narrays(0), nplanes(0), size(0), iterdepth(
				0), idx(0)
{
}

NAryMatIterator::NAryMatIterator(const Mat** _arrays, Mat* _planes,
		int _narrays) :
		arrays(0), planes(0), ptrs(0), narrays(0), nplanes(0), size(0), iterdepth(
				0), idx(0)
{
...
NAryMatIterator::NAryMatIterator(const Mat** _arrays, uchar** _ptrs,
		int _narrays) :
		arrays(0), planes(0), ptrs(0), narrays(0), nplanes(0), size(0), iterdepth(
				0), idx(0)
{
...
void NAryMatIterator::init(const Mat** _arrays, Mat* _planes, uchar** _ptrs,
		int _narrays)
{
...
{
	if (!m)
		return Point();CV_DbgAssert(m->dims <= 2);
...
void MatConstIterator::seek(ptrdiff_t ofs, bool relative)
{
	...
	if (d == 2)
	{
		...
		ptr = y < 0 ? sliceStart :
				y >= m->rows ?
						sliceEnd : sliceStart + (ofs - y * m->cols) * elemSize;
		return;
	}
