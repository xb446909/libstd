...
Mat _InputArray::getMat_(int i) const
{
	...
	{
		int t = type(i);
		const std::vector<std::vector<uchar> >& vv = *(const std::vector<
...
void _InputArray::getMatVector(std::vector<Mat>& mv) const
{
	...
	if (k == MAT)
	{
		...
		for (int i = 0; i < n; i++)
			mv[i] = m.dims == 2 ?
					Mat(1, m.cols, m.type(), (void*) m.ptr(i)) :
					Mat(m.dims - 1, &m.size[1], m.type(), (void*) m.ptr(i),
							&m.step[1]);
		return;
	}
	...
	if (k == MATX || k == STD_ARRAY)
	{
		...
		for (size_t i = 0; i < n; i++)
			mv[i] = Mat(1, sz.width, CV_MAT_TYPE(flags),
					(uchar*) obj + esz * sz.width * i);
		return;
	}
...
Size _InputArray::size(int i) const
{
	...
	if (k == STD_VECTOR)
	{
		...
		return szb == szi ?
				Size((int) szb, 1) : Size((int) (szb / CV_ELEM_SIZE(flags)), 1);
	}
	...
	if (k == STD_VECTOR_VECTOR)
	{
		...
		const std::vector<std::vector<int> >& ivv = *(const std::vector<
				std::vector<int> >*) obj;
	...
	if (k == STD_VECTOR_CUDA_GPU_MAT)
	{
		const std::vector<cuda::GpuMat>& vv =
				*(const std::vector<cuda::GpuMat>*) obj;
		if (i < 0)
			return vv.empty() ? Size() : Size((int) vv.size(), 1);
		CV_Assert(i < (int )vv.size());
		return vv[i].size();
...
int _InputArray::type(int i) const
{
	...
	if (k == MATX || k == STD_VECTOR || k == STD_ARRAY || k == STD_VECTOR_VECTOR
			|| k == STD_BOOL_VECTOR)
...
bool _InputArray::isContinuous(int i) const
{
	...
	if (k == EXPR || k == MATX || k == STD_VECTOR || k == STD_ARRAY || k == NONE
			|| k == STD_VECTOR_VECTOR || k == STD_BOOL_VECTOR)
		return true;
...
void _InputArray::copyTo(const _OutputArray& arr,
...
void _OutputArray::create(int _rows, int _cols, int mtype, int i,
		bool allowTransposed, int fixedDepthMask) const
{
	...
	if (k == CUDA_GPU_MAT && i < 0 && !allowTransposed && fixedDepthMask == 0)
	{
		CV_Assert(
				!fixedSize()
						|| ((cuda::GpuMat* )obj)->size() == Size(_cols, _rows));
		CV_Assert(!fixedType() || ((cuda::GpuMat* )obj)->type() == mtype);
	...
	if (k == OPENGL_BUFFER && i < 0 && !allowTransposed && fixedDepthMask == 0)
	{
		CV_Assert(
				!fixedSize()
						|| ((ogl::Buffer* )obj)->size() == Size(_cols, _rows));
		CV_Assert(!fixedType() || ((ogl::Buffer* )obj)->type() == mtype);
		...
		bool allowTransposed, int fixedDepthMask) const
{
	...
	if (k == MAT)
	{
		...
		if (allowTransposed)
		{
			...
			if (d == 2 && m.dims == 2 && m.data && m.type() == mtype
					&& m.rows == sizes[1] && m.cols == sizes[0])
				return;
		}

		if (fixedType())
		{
			if (CV_MAT_CN(mtype) == m.channels()
					&& ((1 << CV_MAT_TYPE(flags)) & fixedDepthMask) != 0)
				mtype = m.type();
			else
	...
	if (k == UMAT)
	{
		...
		if (allowTransposed)
		{
			...
			if (d == 2 && m.dims == 2 && !m.empty() && m.type() == mtype
					&& m.rows == sizes[1] && m.cols == sizes[0])
				return;
		}
		...
		CV_Assert(
				d == 2
						&& ((sizes[0] == sz.height && sizes[1] == sz.width)
								|| (allowTransposed && sizes[0] == sz.width
										&& sizes[1] == sz.height)));
		return;
	}
	...
	if (k == STD_VECTOR || k == STD_VECTOR_VECTOR)
	{
		...
		if (k == STD_VECTOR_VECTOR)
		{
			std::vector<std::vector<uchar> >& vv = *(std::vector<
					std::vector<uchar> >*) obj;
			if (i < 0)
			{
	...
	if (k == STD_VECTOR_UMAT)
	{
		...
		if (allowTransposed)
		{
			...
			if (d == 2 && m.dims == 2 && m.u && m.type() == mtype
					&& m.rows == sizes[1] && m.cols == sizes[0])
				return;
		}
...
void _OutputArray::setTo(const _InputArray& arr, const _InputArray & mask) const
{
	...
	else if (k == CUDA_GPU_MAT)
	{
		...
		CV_Assert(
				checkScalar(value, type(), arr.kind(),
						_InputArray::CUDA_GPU_MAT));
		((cuda::GpuMat*) obj)->setTo(
				Scalar(Vec<double, 4>(value.ptr<double>())), mask);
	}
	else
		CV_Error(Error::StsNotImplemented, "");
}
