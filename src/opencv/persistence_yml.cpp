...
static char* icvYMLSkipSpaces(CvFileStorage* fs, char* ptr, int min_indent,
		int max_comment_indent)
{
	for (;;)
	{
		while (*ptr == ' ')
...
static void icvYMLGetMultilineStringContent(CvFileStorage* fs, char* ptr,
		int indent, char* &beg, char* &end)
{
...
static char* icvYMLParseBase64(CvFileStorage* fs, char* ptr, int indent,
		CvFileNode * node)
{
	char * beg = 0;
	char * end = 0;
	...
	std::string dt;
	{
		...
		base64::base64_decode(beg, header.data(), 0U,
				base64::ENCODED_HEADER_SIZE);
	...
	if (base64_buffer.empty()
			|| !base64::base64_valid(base64_buffer.data(), 0U,
					base64_buffer.size()))
		CV_PARSE_ERROR("Invalid Base64 data.");

	/* buffer for decoded data(exclude header) */
	std::vector<uchar> binary_buffer(
			base64::base64_decode_buffer_size(base64_buffer.size()));
	int total_byte_size = static_cast<int>(base64::base64_decode_buffer_size(
			base64_buffer.size(), base64_buffer.data(), false));
	{
		base64::Base64ContextParser parser(binary_buffer.data(),
				binary_buffer.size());
		const uchar * buffer_beg =
				reinterpret_cast<const uchar *>(base64_buffer.data());
	...
	base64::make_seq(binary_buffer.data(), elem_cnt, dt.c_str(),
			*node->data.seq);
...
static char* icvYMLParseKey(CvFileStorage* fs, char* ptr, CvFileNode* map_node,
		CvFileNode** value_placeholder)
{
	char c;
		...
		int parent_flags, int min_indent)
{
			...
			|| (c == '.' && cv_isalnum(d))) // a number
	{
		...
		if (*endptr == '.' || *endptr == 'e')
		{
			force_real: fval = icv_strtod(fs, ptr, &endptr);
		...
		{
			force_int: ival = (int) strtol(ptr, &endptr, 0);
			...
			for (len = 0; len < CV_FS_MAX_LEN;)
			{
				...
				else if (c == '\\')
				{
					...
					else if (d == 'x' || (cv_isdigit(d) && d < '8'))
					{
						...
						c = ptr[3];
						ptr[3] = '\0';
						val = (int) strtol(ptr + is_hex, &endptr,
								is_hex ? 8 : 16);
						ptr[3] = c;
	...
	else if (c == '[' || c == '{') // collection as a flow
	{
		...
		int struct_flags = CV_NODE_FLOW
				+ (c == '{' ? CV_NODE_MAP : CV_NODE_SEQ);
		bool is_simple = true;

		icvFSCreateCollection(fs,
		CV_NODE_TYPE(struct_flags) + (node->info ? CV_NODE_USER : 0), node);
	...
	else
	{
		...
		if (is_parent_flow || c != '-')
		{
			...
			if (is_parent_flow || c != ':')
			{
				...
				while (str_end > ptr && c == ' ');
				str_end++;
				node->data.str = cvMemStorageAllocString(fs->memstorage, ptr,
						(int) (str_end - ptr));
				ptr = endptr;
				return ptr;
			}
		...
		icvFSCreateCollection(fs,
				struct_flags + (node->info ? CV_NODE_USER : 0), node);
		...
		int struct_flags, const char* type_name)
{
	...
	struct_flags = (struct_flags & (CV_NODE_TYPE_MASK | CV_NODE_FLOW))
			| CV_NODE_EMPTY;
	if (!CV_NODE_IS_COLLECTION(struct_flags))
		CV_Error(CV_StsBadArg,
...
void icvYMLEndWriteStruct(CvFileStorage* fs)
{
	...
	if (CV_NODE_IS_FLOW(struct_flags))
	{
		...
		if (ptr
				> fs->buffer_start
						+ fs->struct_indent&& !CV_NODE_IS_EMPTY(struct_flags))
			*ptr++ = ' ';
		...
		int quote)
{
	...
	if (quote || len == 0 || str[0] != str[len - 1]
			|| (str[0] != '\"' && str[0] != '\''))
	{
		...
		*data++ = '\"';
		for (i = 0; i < len; i++)
		{
			...
			if (!need_quote && !cv_isalnum(c) && c != '_' && c != ' '
					&& c != '-' && c != '(' && c != ')' && c != '/' && c != '+'
					&& c != ';')
				need_quote = 1;
...
void icvYMLWriteComment(CvFileStorage* fs, const char* comment, int eol_comment)
{
	...
	if (!eol_comment || multiline || fs->buffer_end - ptr < len
			|| ptr == fs->buffer_start)
		ptr = icvFSFlush(fs);
	else
		*ptr++ = ' ';
