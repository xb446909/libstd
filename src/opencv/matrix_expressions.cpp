...
public:
	...
	bool elementWise(const MatExpr& /*expr*/) const
	{
		return true;
	}
...
public:
	...
	static void makeExpr(MatExpr& res, const Mat& a, const Mat& b, double alpha,
			double beta, const Scalar& s = Scalar());
};
...
public:
	...
	static void makeExpr(MatExpr& res, int method, Size sz, int type,
			double alpha = 1);
	static void makeExpr(MatExpr& res, int method, int ndims, const int* sizes,
			int type, double alpha = 1);
};
...
static inline bool isIdentity(const MatExpr& e)
{
	return e.op == &g_MatOp_Identity;
}
static inline bool isAddEx(const MatExpr& e)
{
	return e.op == &g_MatOp_AddEx;
}
static inline bool isScaled(const MatExpr& e)
{
	return isAddEx(e) && (!e.b.data || e.beta == 0) && e.s == Scalar();
}
static inline bool isBin(const MatExpr& e, char c)
{
	return e.op == &g_MatOp_Bin && e.flags == c;
}
static inline bool isCmp(const MatExpr& e)
{
	return e.op == &g_MatOp_Cmp;
}
static inline bool isReciprocal(const MatExpr& e)
{
	return isBin(e, '/') && (!e.b.data || e.beta == 0);
}
static inline bool isT(const MatExpr& e)
{
	return e.op == &g_MatOp_T;
}
static inline bool isInv(const MatExpr& e)
{
	return e.op == &g_MatOp_Invert;
}
static inline bool isSolve(const MatExpr& e)
{
	return e.op == &g_MatOp_Solve;
}
static inline bool isGEMM(const MatExpr& e)
{
	return e.op == &g_MatOp_GEMM;
}
static inline bool isMatProd(const MatExpr& e)
{
	return e.op == &g_MatOp_GEMM && (!e.c.data || e.beta == 0);
}
static inline bool isInitializer(const MatExpr& e)
{
	return e.op == getGlobalMatOpInitializer();
}
...
void MatOp::roi(const MatExpr& expr, const Range& rowRange,
		const Range& colRange, MatExpr& e) const
{
	if (elementWise(expr))
	{
		e = MatExpr(expr.op, expr.flags, Mat(), Mat(), Mat(), expr.alpha,
				expr.beta, expr.s);
		if (expr.a.data)
...
void MatOp::multiply(const MatExpr& e1, const MatExpr& e2, MatExpr& res,
		double scale) const
{
	CV_INSTRUMENT_REGION()
...
void MatOp::divide(const MatExpr& e1, const MatExpr& e2, MatExpr& res,
		double scale) const
{
	CV_INSTRUMENT_REGION()
...
{
	return !expr.a.empty() ? expr.a.size() :
			expr.b.empty() ? expr.b.size() : expr.c.size();
}
...
MatExpr::MatExpr(const Mat& m) :
		op(&g_MatOp_Identity), flags(0), a(m), b(Mat()), c(Mat()), alpha(1), beta(
				0), s(Scalar())
{
}
...
{
	MatExpr e;
	op->roi(*this, Range(roi.y, roi.y + roi.height),
			Range(roi.x, roi.x + roi.width), e);
	return e;
}
...
void MatOp_AddEx::subtract(const Scalar& s, const MatExpr& e,
		MatExpr& res) const
{
	CV_INSTRUMENT_REGION()
...
inline void MatOp_AddEx::makeExpr(MatExpr& res, const Mat& a, const Mat& b,
		double alpha, double beta, const Scalar& s)
{
...
inline void MatOp_Bin::makeExpr(MatExpr& res, char op, const Mat& a,
		const Mat& b, double scale)
{
...
inline void MatOp_Cmp::makeExpr(MatExpr& res, int cmpop, const Mat& a,
		const Mat& b)
{
...
void MatOp_GEMM::subtract(const MatExpr& e1, const MatExpr& e2,
		MatExpr& res) const
{
	CV_INSTRUMENT_REGION()
...
inline void MatOp_GEMM::makeExpr(MatExpr& res, int flags, const Mat& a,
		const Mat& b, double alpha, const Mat& c, double beta)
{
...
void MatOp_Invert::matmul(const MatExpr& e1, const MatExpr& e2,
		MatExpr& res) const
{
...
inline void MatOp_Solve::makeExpr(MatExpr& res, int method, const Mat& a,
		const Mat& b)
{
...
inline void MatOp_Initializer::makeExpr(MatExpr& res, int method, Size sz,
		int type, double alpha)
{
	res = MatExpr(getGlobalMatOpInitializer(), method,
			Mat(sz, type, (void*) (size_t) 0xEEEEEEEE), Mat(), Mat(), alpha, 0);
}

inline void MatOp_Initializer::makeExpr(MatExpr& res, int method, int ndims,
		const int* sizes, int type, double alpha)
{
	res = MatExpr(getGlobalMatOpInitializer(), method,
			Mat(ndims, sizes, type, (void*) (size_t) 0xEEEEEEEE), Mat(), Mat(),
			alpha, 0);
}
