...
public:
	...
	explicit inline lock_guard(Mutex &m) :
			mutex_(&m)
	{
		mutex_->lock();
	}
...
public:
	...
	explicit inline shared_lock_guard(Mutex &m) :
			mutex_(&m)
	{
		mutex_->lock_shared();
	}
...
public:
	...
	explicit inline optional_lock_guard(Mutex* m) :
			mutex_(m)
	{
		if (mutex_)
			mutex_->lock();
...
public:
	...
	explicit inline optional_shared_lock_guard(Mutex* m) :
			mutex_(m)
	{
		if (mutex_)
			mutex_->lock_shared();
