...
static void icvXMLGetMultilineStringContent(CvFileStorage* fs, char* ptr,
		char* &beg, char* &end)
{
...
static char*
icvXMLParseValue(CvFileStorage* fs, char* ptr, CvFileNode* node, int value_type
CV_DEFAULT(CV_NODE_NONE))
{
			...
			else
			{
				...
				for (;;)
				{
					...
					if (!cv_isalnum(c))
					{
						...
						else if (!cv_isprint(c) || c == '<'
								|| (!is_quoted && cv_isspace(c)))
						{
							if (is_quoted)
						...
						else if (c == '&')
						{
							if (*++ptr == '#')
							{
								...
								if ((unsigned) val > (unsigned) 255 || !endptr
										|| *endptr != ';')
									CV_PARSE_ERROR(
							...
							else
							{
								...
								if (len == 2 && memcmp(ptr, "lt", len) == 0)
									...
									c = '>';
										...
										&& memcmp(ptr, "amp", len) == 0)
									c = '&';
								else if (len == 4
										&& memcmp(ptr, "apos", len) == 0)
									c = '\'';
								else if (len == 4
										&& memcmp(ptr, "quot", len) == 0)
									c = '\"';
								else
								{
				...
				elem->data.str = cvMemStorageAllocString(fs->memstorage, buf,
						i);
			}
		...
		CvAttrList** _list, int* _tag_type)
{
	...
	int attr_buf_size = (max_count * 2 + 1) * sizeof(char*)
			+ sizeof(CvAttrList);
	char* endptr;
	char c;
	int have_space;
	...
	for (;;)
	{
		...
		else
		{
			...
			if (!last || count >= max_count)
			{
				...
				chunk = (CvAttrList*) cvMemStorageAlloc(fs->memstorage,
						attr_buf_size);
...
void icvXMLParse(CvFileStorage* fs)
{
	...
	while (*ptr != '\0')
	{
		...
		if (*ptr != '\0')
		{
			...
			ptr = icvXMLParseTag(fs, ptr, &key, &list, &tag_type);
			if (tag_type != CV_XML_OPENING_TAG || !key
...
static void icvXMLWriteTag(CvFileStorage* fs, const char* key, int tag_type,
		CvAttrList list)
{
	char* ptr = fs->buffer;
	int i, len = 0;
		...
		int struct_flags, const char* type_name)
{
	...
	if (key)
	{
		fs->struct_tag = cvMemStorageAllocString(fs->strstorage, (char*) key,
				-1);
	}
	else
	{
...
void icvXMLEndWriteStruct(CvFileStorage* fs)
{
	...
	icvXMLWriteTag(fs, fs->struct_tag.ptr, CV_XML_CLOSING_TAG,
			cvAttrList(0, 0));
		...
		int len)
{
	...
	else
	{
		...
		if ((new_offset > fs->wrap_margin && new_offset - fs->struct_indent > 10)
				|| (ptr > fs->buffer_start && ptr[-1] == '>'
						&& !CV_NODE_IS_EMPTY(fs->struct_flags)))
		{
			ptr = icvXMLFlush(fs);
		}
...
void icvXMLWriteComment(CvFileStorage* fs, const char* comment, int eol_comment)
{
	...
	if (multiline)
	{
		while (comment)
		{
			if (eol)
			{
				ptr = icvFSResizeWriteBuffer(fs, ptr,
						(int) (eol - comment) + 1);
