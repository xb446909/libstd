...
static Mat cvMatToMat(const CvMat* m, bool copyData)
{
	...
	if (!copyData)
	{
		thiz.flags = Mat::MAGIC_VAL
				+ (m->type & (CV_MAT_TYPE_MASK | CV_MAT_CONT_FLAG));
		thiz.dims = 2;
		thiz.rows = m->rows;
		thiz.cols = m->cols;
		thiz.datastart = thiz.data = m->data.ptr;
		size_t esz = CV_ELEM_SIZE(m->type), minstep = thiz.cols * esz, _step =
				m->step;
		if (_step == 0)
			_step = minstep;
...
static Mat iplImageToMat(const IplImage* img, bool copyData)
{
	...
	else
	{
		...
		bool selectedPlane = img->roi->coi
				&& img->dataOrder == IPL_DATA_ORDER_PLANE;
		m.flags = Mat::MAGIC_VAL
				+ CV_MAKETYPE(imgdepth, selectedPlane ? 1 : img->nChannels);
		m.rows = img->roi->height;
		m.cols = img->roi->width;
		esz = CV_ELEM_SIZE(m.flags);
		m.datastart = m.data = (uchar*) img->imageData
				+ (selectedPlane ?
						(img->roi->coi - 1) * m.step * img->height : 0)
	...
	m.flags |= (
			m.cols * esz == m.step.p[0] || m.rows == 1 ?
		...
		AutoBuffer<double>* abuf)
{
	...
	{
		CvSeq* seq = (CvSeq*) arr;
		int total = seq->total, type = CV_MAT_TYPE(seq->flags), esz =
				seq->elem_size;
		if (total == 0)
			return Mat();
		...
		{
			abuf->allocate(
					((size_t) total * esz + sizeof(double) - 1)
							/ sizeof(double));
			double* bufdata = *abuf;
...
void insertImageCOI(InputArray _ch, CvArr* arr, int coi)
{
	...
	CV_Assert(
			ch.size == mat.size && ch.depth() == mat.depth() && 0 <= coi
					&& coi < mat.channels());
	int _pairs[] =
	{ 0, coi };
...
CV_IMPL void cvTranspose(const CvArr* srcarr, CvArr* dstarr)
{
	...
	CV_Assert(
			src.rows == dst.cols && src.cols == dst.rows
					&& src.type() == dst.type());
	transpose(src, dst);
}
...
CV_IMPL void cvCrossProduct(const CvArr* srcAarr, const CvArr* srcBarr,
		CvArr* dstarr)
{
...
CV_IMPL void cvReduce(const CvArr* srcarr, CvArr* dstarr, int dim, int op)
{
	...
	if (dim > 1)
		CV_Error(CV_StsOutOfRange,
				"The reduced dimensionality index is out of range");
	...
	if (src.channels() != dst.channels())
		CV_Error(CV_StsUnmatchedFormats,
				"Input and output arrays must have the same number of channels");
...
CV_IMPL void cvSort(const CvArr* _src, CvArr* _dst, CvArr* _idx, int flags)
{
	...
	if (_idx)
	{
		...
		CV_Assert(
				src.size() == idx.size() && idx.type() == CV_32S && src.data != idx.data);
...
CV_IMPL int cvKMeans2(const CvArr* _samples, int cluster_count, CvArr* _labels,
		CvTermCriteria termcrit, int attempts, CvRNG*, int flags,
		CvArr* _centers, double* _compactness)
{
	...
	double compactness = cv::kmeans(data, cluster_count, labels, termcrit,
			attempts, flags,
