...
template<typename _Tp> static inline int LUImpl(_Tp* A, size_t astep, int m,
		_Tp* b, size_t bstep, int n, _Tp eps)
{
	int i, j, k, p = 1;
	astep /= sizeof(A[0]);
	bstep /= sizeof(b[0]);
...
template<typename _Tp> static inline bool CholImpl(_Tp* A, size_t astep, int m,
		_Tp* b, size_t bstep, int n)
{
	_Tp* L = A;
	int i, j, k;
	double s;
...
template<typename _Tp> inline static int sign(_Tp x)
{
	if (x >= (_Tp) 0)
		return 1;
	else
...
template<typename _Tp> static inline int QRImpl(_Tp* A, size_t astep, int m,
		int n, int k, _Tp* b, size_t bstep, _Tp* hFactors, _Tp eps)
{
	astep /= sizeof(_Tp);
	bstep /= sizeof(_Tp);
