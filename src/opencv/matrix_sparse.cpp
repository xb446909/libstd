...
template<typename T1, typename T2> void convertData_(const void* _from,
		void* _to, int cn)
{
...
template<typename T1, typename T2> void convertScaleData_(const void* _from,
		void* _to, int cn, double alpha, double beta)
{
	const T1* from = (const T1*) _from;
	T2* to = (T2*) _to;
	if (cn == 1)
	...
	static ConvertData tab[][8] =
			...
			{ convertData_<short, uchar>, convertData_<short, schar>,
					...
					{ convertData_<float, uchar>, convertData_<float, schar>,
							convertData_<float, ushort>, convertData_<float,
									short>, convertData_<float, int>,
							convertData_<float, float>, convertData_<float,
									double>, 0 },

					{ convertData_<double, uchar>, convertData_<double, schar>,
							convertData_<double, ushort>, convertData_<double,
									short>, convertData_<double, int>,
							convertData_<double, float>, convertData_<double,
									double>, 0 },
...
SparseMat::Hdr::Hdr(int _dims, const int* _sizes, int _type)
{
	...
	dims = _dims;
	valueOffset = (int) alignSize(
			sizeof(SparseMat::Node) - MAX_DIM * sizeof(int)
					+ dims * sizeof(int), CV_ELEM_SIZE1(_type));
	nodeSize = alignSize(valueOffset + CV_ELEM_SIZE(_type),
			(int) sizeof(size_t));
		...
		flags(MAGIC_VAL), hdr(0)
{
	...
	int i, idx[CV_MAX_DIM] =
	{ 0 }, d = m.dims, lastSize = m.size[d - 1];
...
uchar* SparseMat::ptr(int i0, int i1, int i2, bool createMissing,
		size_t* hashval)
{
...
uchar* SparseMat::newNode(const int* idx, size_t hashval)
{
	...
	if (!hdr->freeList)
	{
		size_t i, nsz = hdr->nodeSize, psize = hdr->pool.size(), newpsize =
				std::max(psize * 3 / 2, 8 * nsz);
...
void minMaxLoc(const SparseMat& src, double* _minval, double* _maxval,
		int* _minidx, int* _maxidx)
{
	CV_INSTRUMENT_REGION()
