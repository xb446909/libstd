...
icvJSONSkipSpaces(CvFileStorage* fs, char* ptr)
{
	...
	while (is_eof == false && is_completed == false)
	{
		...
		case '/':
		{
			...
			if (*ptr == '\0')
			{
				ptr = icvGets(fs, fs->buffer_start,
						static_cast<int>(fs->buffer_end - fs->buffer_start));
				if (!ptr)
				{
					is_eof = true;
					break;
...
static char* icvJSONParseKey(CvFileStorage* fs, char* ptr, CvFileNode* map,
		CvFileNode** value_placeholder)
{
	...
	{
		CvStringHashNode* str_hash_node = cvGetHashedKey(fs, beg,
				static_cast<int>(end - beg), 1);
...
static char* icvJSONParseValue(CvFileStorage* fs, char* ptr, CvFileNode* node)
{
	...
	{ /* must be string or Base64 string */
		...
		{ /**************** Base64 string ****************/
			...
			if (base64_buffer.size() >= base64::ENCODED_HEADER_SIZE)
			{
				...
				std::string dt;
				{
					...
					base64::base64_decode(base64_beg, header.data(), 0U,
				...
				if (base64_buffer.size() > base64::ENCODED_HEADER_SIZE)
				{
					if (!base64::base64_valid(base64_beg, 0U,
							base64_end - base64_beg))
					...
					std::vector<uchar> binary_buffer(
							base64::base64_decode_buffer_size(
									base64_end - base64_beg));
					int total_byte_size =
							static_cast<int>(base64::base64_decode_buffer_size(
									base64_end - base64_beg, base64_beg, false));
					{
						base64::Base64ContextParser parser(binary_buffer.data(),
								binary_buffer.size());
						const uchar * binary_beg =
								reinterpret_cast<const uchar *>(base64_beg);
						const uchar * binary_end = binary_beg
								+ (base64_end - base64_beg);
		...
		{ /**************** normal string ****************/
			...
			node->data.str = cvMemStorageAllocString(fs->memstorage,
					string_buffer.c_str(),
...
void icvJSONWrite(CvFileStorage* fs, const char* key, const char* data)
{
	...
	if (CV_NODE_IS_FLOW(struct_flags))
	{
		...
		if (!CV_NODE_IS_EMPTY(struct_flags))
			*ptr++ = ',';
		new_offset = static_cast<int>(ptr - fs->buffer_start + key_len
				+ data_len);
