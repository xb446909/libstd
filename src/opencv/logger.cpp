...
{
	static cv::String param_log_level = utils::getConfigurationParameterString(
			"OPENCV_LOG_LEVEL", "INFO");
	if (param_log_level == "DISABLED" || param_log_level == "disabled"
			|| param_log_level == "0" || param_log_level == "OFF"
			|| param_log_level == "off")
	...
	std::cerr << "ERROR: Unexpected logging level value: " << param_log_level
			<< std::endl;
	return LOG_LEVEL_INFO;
}
...
void writeLogMessage(LogLevel logLevel, const char* message)
{
	...
	std::ostream* out =
			(logLevel <= LOG_LEVEL_WARNING) ? &std::cerr : &std::cout;
	std::stringstream ss;
	switch (logLevel)
	{
