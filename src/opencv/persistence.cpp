		...
		CvMemStorage* storage, int start_tab_size)
{
	...
	CvGenericHash* map = (CvGenericHash*) cvCreateSet(flags, header_size,
			elem_size, storage);
...
static void icvProcessSpecialDouble(CvFileStorage* fs, char* buf, double* value,
		char** endptr)
{
	char c = buf[0];
	...
	v.d = 0.;
	if (toupper(buf[1]) == 'I' && toupper(buf[2]) == 'N'
			&& toupper(buf[3]) == 'F')
		v.i = (uint64) inf_hi << 32;
	else if (toupper(buf[1]) == 'N' && toupper(buf[2]) == 'A'
			&& toupper(buf[3]) == 'N')
		v.i = (uint64) -1;
	else
		CV_PARSE_ERROR("Bad format of floating-point constant");
...
{
	const char * err_unkonwn_state =
			"Unexpected error, unable to determine the Base64 state.";
	const char * err_unable_to_switch =
		...
		bool change_type_to_base64)
{
	if (fs->is_write_struct_delayed)
	{
		...
		{
			fs->start_write_struct(fs, struct_key.c_str(), struct_flags,
					"binary");
...
{
	CV_Assert(fs->is_write_struct_delayed == false);CV_DbgAssert( fs->delayed_struct_key == 0 );CV_DbgAssert( fs->delayed_struct_flags == 0 );CV_DbgAssert( fs->delayed_type_name == 0 );
...
{
	sprintf(dt, "%d%c", CV_MAT_CN(elem_type),
			icvTypeSymbol(CV_MAT_DEPTH(elem_type)));
		...
		const CvFileNode* node)
{
	...
	case CV_NODE_MAP:
		...
		CV_NODE_TYPE(node->tag) + (CV_NODE_SEQ_IS_SIMPLE(node->data.seq) ?
		CV_NODE_FLOW :
																			0),
