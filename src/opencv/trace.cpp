...
static bool param_traceEnable = utils::getConfigurationParameterBool(
		"OPENCV_TRACE", false);

static int param_maxRegionDepthOpenCV =
		(int) utils::getConfigurationParameterSizeT("OPENCV_TRACE_DEPTH_OPENCV",
				1);
static int param_maxRegionChildrenOpenCV =
		(int) utils::getConfigurationParameterSizeT(
				"OPENCV_TRACE_MAX_CHILDREN_OPENCV", 1000);
static int param_maxRegionChildren =
		(int) utils::getConfigurationParameterSizeT("OPENCV_TRACE_MAX_CHILDREN",
				10000);
static cv::String param_traceLocation = utils::getConfigurationParameterString(
		"OPENCV_TRACE_LOCATION", "OpenCVTrace");
...
public:
	...
	bool formatRegionEnter(const Region& region)
	{
		...
		{
			if (region.pImpl->parentRegion->pImpl->threadID
					!= region.pImpl->threadID)
				ok &=
						this->printf(",parentThread=%d,parent=%lld",
	...
	bool formatRegionLeave(const Region& region, const RegionStatistics& result)
	{
		...
		bool ok =
				this->printf("e,%d,%lld,%lld,%lld,%lld",
						(int) region.pImpl->threadID,
	...
	bool recordRegionArg(const Region& region, const TraceArg& arg,
			const char* value)
	{
...
Region::LocationExtraData::LocationExtraData(
		const LocationStaticStorage& location)
{
	...
	CV_LOG(
			"Register location: " << global_location_id << " (" << (void*)&location << ")" << std::endl << "    file: " << location.filename << std::endl << "    line: " << location.line << std::endl << "    name: " << location.name);
#ifdef OPENCV_WITH_ITT
	if (isITTEnabled())
	{
...
/*static*/Region::LocationExtraData* Region::LocationExtraData::init(
...
Region::Impl::Impl(TraceManagerThreadLocal& ctx, Region* parentRegion_,
		Region& region_, const LocationStaticStorage& location_,
		int64 beginTimestamp_) :
		location(location_), region(region_), parentRegion(parentRegion_), threadID(
				ctx.threadID), global_region_id(++ctx.region_counter), beginTimestamp(
				beginTimestamp_), endTimestamp(0), directChildrenCount(0)
#ifdef OPENCV_WITH_ITT
...
void Region::Impl::leaveRegion(TraceManagerThreadLocal& ctx)
{
	...
	CV_LOG(
			_spaces(ctx.getCurrentDepth()*4) << "leaveRegion(): " << (void*)this << " " << result);
#ifdef OPENCV_WITH_ITT
	if (isITTEnabled())
	{
		if (result.currentSkippedRegions)
...
void RegionStatisticsStatus::checkResetSkipMode(int leaveDepth)
{
	...
	{
		CV_LOG_SKIP(NULL,
				"SKIP-RESET: leaveDepth=" << leaveDepth << " skipDepth=" << _skipDepth);
		_skipDepth = -1;
	}
}

Region::Region(const LocationStaticStorage& location) :
		pImpl(NULL), implFlags(0)
{
	// Checks:
	// - global enable flag
	...
	if (parentRegion && parentRegion->pImpl)
	{
		if (parentLocation == NULL)
		{
			...
			parentChildren =
			CV_XADD(&parentRegion->pImpl->directChildrenCount, 1) + 1;
		}
		else
		{
	...
	{
		if (ctx.stat_status._skipDepth >= 0
				&& currentDepth > ctx.stat_status._skipDepth)
		{
			CV_LOG(
					_spaces(ctx.getCurrentDepth()*4) << "Parent region is disabled. Bailout");
			ctx.stat.currentSkippedRegions++;
			return;
		}

		if (param_maxRegionChildrenOpenCV > 0
				&& (location.flags & REGION_FLAG_APP_CODE) == 0
				&& parentLocation
				&& (parentLocation->flags & REGION_FLAG_APP_CODE) == 0)
		{
			if (parentChildren >= param_maxRegionChildrenOpenCV)
			{
				CV_LOG_TRACE_BAILOUT(NULL,
						_spaces(ctx.getCurrentDepth()*4) << "OpenCV parent region exceeds children count. Bailout");
		...
		if (param_maxRegionChildren > 0
				&& parentChildren >= param_maxRegionChildren)
		{
			CV_LOG_TRACE_BAILOUT(NULL,
					_spaces(ctx.getCurrentDepth()*4) << "Parent region exceeds children count. Bailout");
			ctx.stat_status.enableSkipMode(currentDepth - 1);
			ctx.stat.currentSkippedRegions++;
			DEBUG_ONLY(ctx.dumpStack(std::cout, false));
			return;
	...
	{
		CV_LOG_TRACE_BAILOUT(NULL,
				_spaces(ctx.getCurrentDepth()*4) << "Region location is disabled. Bailout");
	...
	{
		CV_LOG(
				_spaces(ctx.getCurrentDepth()*4) << "Parent region disables inner regions. Bailout");
		ctx.stat_status.enableSkipMode(currentDepth);
		ctx.stat.currentSkippedRegions++;
		return;
	}
	...
	if (param_maxRegionDepthOpenCV)
	{
		if ((location.flags & REGION_FLAG_APP_CODE) == 0)
		{
			...
			{
				CV_LOG(
						_spaces(ctx.getCurrentDepth()*4) << "OpenCV region depth is exceed = " << ctx.regionDepthOpenCV << ". Bailout");
...
void Region::destroy()
{
	...
	TraceManagerThreadLocal& ctx = getTraceManager().tls.getRef();
	CV_LOG(
			_spaces(ctx.getCurrentDepth()*4) << "Region::destruct(): " << (void*)this << " pImpl=" << pImpl << " implFlags=" << implFlags << ' ' << (ctx.stackTopLocation() ? ctx.stackTopLocation()->name : "<unknown>"));

	CV_DbgAssert(implFlags & REGION_FLAG__NEED_STACK_POP);
	const int currentDepth = ctx.getCurrentDepth();
	CV_UNUSED(currentDepth);

	CV_LOG_CTX_STAT(NULL,
			_spaces(currentDepth*4) << ctx.stat << ' ' << ctx.stat_status);
	...
	if (pImpl)
	{
		CV_DbgAssert((implFlags & (REGION_FLAG__ACTIVE | REGION_FLAG__NEED_STACK_POP)) == (REGION_FLAG__ACTIVE | REGION_FLAG__NEED_STACK_POP));CV_DbgAssert(ctx.stackTopRegion() == this);
...
void TraceManagerThreadLocal::dumpStack(std::ostream& out,
		bool onlyFunctions) const
{
	std::stringstream ss;
...
public:
	...
	AsyncTraceStorage(const std::string& filename) :
			out(filename.c_str(), std::ios::trunc), name(filename)
	{
...
TraceStorage* TraceManagerThreadLocal::getStorage() const
{
	...
	if (storage.empty())
	{
		...
		if (global)
		{
			const std::string filepath = cv::format("%s-%03d.txt",
					param_traceLocation.c_str(), threadID).c_str();
			TraceMessage msg;
...
TraceManager::~TraceManager()
{
	...
	{
		CV_LOG_WARNING(NULL,
				"Trace: Total skipped events: " << totalSkippedEvents);
	}
		...
		const TraceManagerThreadLocal& root_ctx)
{
	...
	ctx.dummy_stack_top = TraceManagerThreadLocal::StackEntry(
			const_cast<Region*>(&rootRegion), NULL, -1);
...
void parallelForAttachNestedRegion(const Region& rootRegion)
{
	...
	CV_LOG_PARALLEL(NULL,
			" PARALLEL_FOR: " << (void*)region << " ==> " << &rootRegion);
	if (!region)
		return;
...
void parallelForFinalize(const Region& rootRegion)
{
	...
	int64 duration = endTimestamp - ctx.stackTopBeginTimestamp();
	CV_LOG_PARALLEL(NULL,
			"parallel_for duration: " << duration << " " << &rootRegion);
	...
	for (size_t i = 0; i < threads_ctx.size(); i++)
	{
		...
		{
			CV_LOG_PARALLEL(NULL,
					"Thread=" << child_ctx->threadID << " " << child_ctx->stat);
			threads++;
			...
			if (child_ctx != &ctx)
			{
				child_ctx->dummy_stack_top =
						TraceManagerThreadLocal::StackEntry();
			}
			else
			{
	...
	float parallel_coeff = std::min(1.0f,
			duration / (float) (parallel_for_stat.duration));
