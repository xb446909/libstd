...
namespace cv
{
	...
	static bool ocl_setIdentity( InputOutputArray _m, const Scalar& s )
	{
		...
		size_t globalsize[2] =
		{	(size_t)m.cols * cn / kercn, ((size_t)m.rows + rowsPerWI - 1) / rowsPerWI};
...
template<typename T> static void transpose_(const uchar* src, size_t sstep,
...
typedef void (*TransposeFunc)(const uchar* src, size_t sstep, uchar* dst,
		size_t dstep, Size sz);
...
{ 0, transpose_8u, transpose_16u, transpose_8uC3, transpose_32s, 0,
		transpose_16uC3, 0, transpose_32sC2, 0, 0, 0, transpose_32sC3, 0, 0, 0,
		transpose_32sC4, 0, 0, 0, 0, 0, 0, 0, transpose_32sC6, 0, 0, 0, 0, 0, 0,
		0, transpose_32sC8 };

static TransposeInplaceFunc transposeInplaceTab[] =
{ 0, transposeI_8u, transposeI_16u, transposeI_8uC3, transposeI_32s, 0,
		transposeI_16uC3, 0, transposeI_32sC2, 0, 0, 0, transposeI_32sC3, 0, 0,
		0, transposeI_32sC4, 0, 0, 0, 0, 0, 0, 0, transposeI_32sC6, 0, 0, 0, 0,
		0, 0, 0, transposeI_32sC8 };
...
static bool ocl_transpose( InputArray _src, OutputArray _dst )
{
	...
	{	TILE_DIM, BLOCK_ROWS};
	size_t globalsize[2] =
	{	(size_t)src.cols, inplace ? ((size_t)src.rows + rowsPerWI - 1) / rowsPerWI : (divUp((size_t)src.rows, TILE_DIM) * BLOCK_ROWS)};
...
cv::Mat cv::Mat::cross(InputArray _m) const
{
	...
	CV_Assert(
			dims <= 2 && m.dims <= 2 && size() == m.size() && tp == m.type()
					&& ((rows == 3 && cols == 1)
							|| (cols * channels() == 3 && rows == 1)));
...
template<typename T, typename ST, class Op> static void reduceR_(
		const Mat& srcmat, Mat& dstmat)
{
...
template<typename T, typename ST, class Op> static void reduceC_(
		const Mat& srcmat, Mat& dstmat)
{
	typedef typename Op::rtype WT;
	Size size = srcmat.size();
...
namespace cv
{
			...
			int dim, int op, int op0, int stype, int dtype)
	{
		...
		const char * const ops[4] =
		{	"OCL_CV_REDUCE_SUM", "OCL_CV_REDUCE_AVG",
		...
		if (useOptimized)
		{
			...
			size_t localSize[2] =
			{	(size_t)buf_cols, (size_t)tileHeight};
			size_t globalSize[2] =
			{	(size_t)buf_cols, (size_t)src.rows};
			return k.run(2, globalSize, localSize, false);
		}
		else
		{
